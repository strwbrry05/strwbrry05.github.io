...
const HeroMobile = (props) => {
  // mouse over mouse in mouse out? state var? use? to connect text and face img
  return (
    <>
      <div className="flex flex-row flex-wrap justify-center gap-y-[1em] md:gap-y-[2em]">
        {/* hobonichi */}
        <div
      ...
      <div
        className={`font-(family-name:--font-text) text-(--color-gray) flex flex-col justify-center items-center
           cursor-default hover:text-black`}
        onMouseEnter={() => props.setIsHovered(true)}
        onMouseLeave={() => props.setIsHovered(false)}
      >
        <div className={`flex flex-col justify-center items-center`}>
          <p className="text-[1.15rem] text-center mb-[0.5em]">edward hopper</p>
          <img className="w-[300px]" src={art} alt="edward hopper:soir bleu" />
        </div>
        <div className="w-[90%] mt-[1em] md:w-[70%]">
          <p>
            i never understood the meaning of ‘art speaks to you’ until i found
            ‘soir bleu’. edward hopper has a fascinating ability to create and
            evoke feelings of isolation and contemplation thorugh his works.
            finding your favorite artist is important, i think. it tells you a
            lot about a person. dont know what this says about me however
          </p>
        </div>
      </div>
      {/* self */}
      {props.isHovered ? (
        ...
        <img src={notA} alt="" className="w-[full] md:w-[300px] self-center" />
      )}
    </>
  );
};
